...
public class LitTemplateInitializer implements Serializable {
    private static final ReflectionCache<LitTemplate, ParserData> CACHE = new ReflectionCache<>(templateClass -> {
        return new LitTemplateDataAnalyzer(templateClass).parseTemplate();
    });
     ...
     *
     * @param template a template to initialize
    ...
    public LitTemplateInitializer(LitTemplate template, VaadinService service) {
        ...
        Class<? extends LitTemplate> templateClass = (Class<? extends LitTemplate>) template.getClass();
